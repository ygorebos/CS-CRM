...
| **VII** — interoperável por contrato | **CONFORME pela v2.3.0**, que nomeou a quarta superfície (função `security definer` versionada) e a cercou com seis travas. FR-001 a FR-004 respondem quatro delas por requisito; T011 (invariante que reprova grant de tabela) e T016 (varredura de HTTP na função) respondem as outras duas — e são **prova, não desenho**. ⚠️ Enquanto T011 e T016 não estiverem verdes, esta linha é promessa auditável: a emenda permitiu a superfície **sob condição**, e sem a condição ela volta a ser proibida. A quarta superfície **não** se estende ao Cotador |
