...
```text
supabase/
├── migrations/                                        # última aplicada em 2026-08-08: 0122 (spec 001)
│   ├── <ts>_0116_aviso_de_assistencia_sem_lastro.sql # F1 — já existe
│   ├── <ts>_0117_catalogo_curado_particao.sql        # F2 — já existe
│   ├── <ts>_0118_escopos_por_tenant_e_vinculo.sql    # F2 — já existe, inclui o eixo no acervo existente
...
1. **A `fn_buscar_lastro` lia colunas de duas migrations à frente.** Ela nasce na 0123 (F2) e filtra
   o acervo do tenant por `scope_id` e por `valid_until` — colunas que a 0124 (F4) criava. A função
