 ...
 *      O manual da operadora entra por `ai_source_passages` (migration 0132). Aceitar o
...
/** Uma linha de `ai_source_passages` (migration 0132) — texto de documento já gravado. */
interface Passagem {
  knowledge_source_id: string;
  content: string;
  position: number;
...
describe("rag-indexer · material que não é par pergunta/resposta (T077, FR-004)", () => {
  it("aceita material que não é par pergunta/resposta e o torna buscável", async () => {
    ...
    // morasse. Com `ai_source_passages` (migration 0132) e o indexador lendo dela, a fonte
