...
export async function duplicateAgentWithVersion(
  ...
  const { data: newAgent, error: agentErr } = await admin
    ...
    .insert({
      ...
      //
      // A coluna é `not null default <rag_must_hit>` (migration 0134): omitir a chave faz o
      ...
      // defeito que a 0134 fechou. Só o SELECT deixar de trazer a coluna já bastaria para
      // acontecer.
