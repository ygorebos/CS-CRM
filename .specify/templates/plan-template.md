...
Fonte: `.specify/memory/constitution.md` v2.3.0. Marque PASS/FAIL/N/A por gate; todo FAIL
...
| VII | Interoperável por contrato | Integração externa consome uma das **quatro** superfícies (API v1 / MCP / webhooks / função versionada) — sem tocar **tabela** do outro sistema nem FK cruzando fronteira de produto? Entidade trocada carrega `organization_id` e é rastreável até o lead? **Se usa a quarta superfície**, as seis travas estão respondidas — zero grant de tabela (nem `select`), papel dedicado e nunca `service_role`, tenant resolvido dentro do banco e nunca de parâmetro, assinatura versionada como contrato, invariante em CI que reprova grant de tabela, e sem HTTP dentro da função? Ela é **só do gateway** — não se estende ao Cotador? | |
