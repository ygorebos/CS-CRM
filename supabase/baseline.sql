...
alter table public.channel_sessions
  ...
  -- migration 0119: ponteiro para a conexão do lado do gateway. Mora AQUI, e não
...
--
-- migration 0119: os quatro últimos espelham `MensagemNormalizada.Platform` do
...
-- errada.
--
-- migration 0119: os canais do gateway se identificam por `gateway_connection_id`,
...
alter table public.agent_inbox_items
  add constraint agent_inbox_items_kind_check check (kind in (
    ...
    -- (migration 0121, spec 001 §T037) Entrega de envelope DESCARTADA pelo dreno
...
-- ---- recebimento unificado pelo gateway (migration 0119) ----
--
...
-- ---- mídia do gateway pede a própria persistência (migration 0139) ----
-- Redefine `fn_gateway_ingest_message` (o bloco da 0128 fica acima; este vem
-- DEPOIS e é o que vale, nos dois modos de aplicação do baseline). O que muda é
-- um bloco: quando `metadata.media_ref` chega preenchido, a função emite
-- `media.persist_requested` na mesma transação do insert. Sem ele, toda mídia
-- que entra pela quarta superfície fica descrita na mensagem e sem arquivo
-- nenhum — o worker que baixa nunca é acordado.
--
-- Idempotente por `create or replace`; a assinatura NÃO muda (trava nº 4 do
-- Princípio VII).
create or replace function public.fn_gateway_ingest_message(
  p_gateway_connection_id text,
  p_external_id           text,
